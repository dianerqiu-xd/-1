...
st.markdown(
    ...
    div[data-testid="stMetric"] { background: #f6f8fb; border: 1px solid #e6e9ef; border-radius: 8px; padding: 10px 12px; }
...
def show_bgr(image: np.ndarray | None, caption: str = "", width: int | None = None) -> None:
    if image is None:
        ...
        return
    if image.ndim == 2:
        st.image(image, caption=caption, clamp=True, width=width)
    else:
        st.image(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), caption=caption, width=width)


def image_picker(label: str, sample_name: str, key: str) -> np.ndarray | None:
    source = st.radio(
        label,
        ["使用内置示例", "上传图片"],
        horizontal=True,
...
with tab_edges:
    ...
    with left:
        edge_image = image_picker("输入图像", "feature_sample.jpg", "edge")
...
with tab_features:
    ...
    with col1:
        feat_image = image_picker("输入图像", "feature_sample.jpg", "feature")
...
with tab_matching:
    ...
    with img_col1:
        match_img1 = image_picker("图像 1", "match_1.jpg", "match1")
    with img_col2:
        match_img2 = image_picker("图像 2", "match_2.jpg", "match2")
...
with tab_panorama:
    ...
    if mode == "两幅图像":
        c1, c2 = st.columns(2)
        with c1:
            pano_1 = image_picker("左/基准图", "pano_left.jpg", "pano1")
        with c2:
            pano_2 = image_picker("右/待拼接图", "pano_mid.jpg", "pano2")
        ...
        samples = [read_asset("pano_left.jpg"), read_asset("pano_mid.jpg"), read_asset("pano_right.jpg")]
        uploaded_many = st.file_uploader(
            "上传多幅同一场景图片，留空则使用内置三图示例",
