...
def harris_features(
    ...
    heat = cv2.normalize(response, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
...
def match_pipeline(
    ...
    if homography is not None:
        transform_view, aligned_view = warp_and_align(img1, img2, homography)
...
def warp_and_align(img1: np.ndarray, img2: np.ndarray, homography: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    translation, (width, height), (x_offset, y_offset) = panorama_bounds(img1, img2, homography)
...
def _canvas_images(img1: np.ndarray, img2: np.ndarray, homography: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    translation, (width, height), (x_offset, y_offset) = panorama_bounds(img1, img2, homography)
    warped_1 = cv2.warpPerspective(img1, translation @ homography, (width, height))
...
def stitch_pair(
    ...
    warped_1, warped_2, mask_1, mask_2 = _canvas_images(resize_for_work(_ensure_bgr(img1)), resize_for_work(_ensure_bgr(img2)), result.homography)
