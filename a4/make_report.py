...
def make_screenshots():
    ...
    query_id = 8
    _, nn, _, _ = app.knn_predict(train_x, train_y, test_x[query_id], 5)
    p2 = save_fig(app.plot_knn_embedding(train_x, train_y, test_x[query_id], nn, 5), "02_knn_k5.png")

    w, _, losses, acc, source = app.train_linear_classifier(60, 13, 260, 0.18, 1e-3)
...
def build_pdf(assets):
    ...
    draw_text_box(
        d,
        (120, y + 32),
        "应用交互模块：\n1. 最小二乘：调节样本数、噪声、真实斜率/截距并实时观察闭式解。\n2. KNN：选择测试图像和 K 值，查看最近邻、投票结果、PCA 可视化。\n3. 线性分类器：训练 softmax 线性分类器并把权重还原成 32x32 RGB 模板。\n4. 优化与 Loss：比较普通梯度下降和动量路径，逐项展示 SVM hinge loss 与 softmax cross-entropy。",
        42,
        size=25,
        fill="#0F172A",
    )
    ...
    draw_text_box(
        d,
        (90, 850),
        "KNN 使用 32x32x3 图像向量的欧氏距离，在交互页中展示 query 图像、最近邻缩略图、不同 K 的投票结果和 PCA 二维邻域。K 变大时分类结果会更平滑，但可能牺牲局部细节。",
        43,
        size=28,
        fill="#334155",
    )
