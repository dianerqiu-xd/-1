...
def page_knn():
    ...
    with left:
        ...
        st.image(test_x[query_id], caption=f"真实类别：{CLASS_NAMES[test_y[query_id]]} / {CLASS_CN[CLASS_NAMES[test_y[query_id]]]}", width=150)
    pred, nn, distances, votes = knn_predict(train_x, train_y, test_x[query_id], k)
    with right:
        st.pyplot(plot_knn_embedding(train_x, train_y, test_x[query_id], nn, k), clear_figure=True)
    ...
    for kk in [1, 3, 5, 7, 9, 15]:
        pp, _, _, vv = knn_predict(train_x, train_y, test_x[query_id], kk)
...
def page_templates():
    ...
    with st.spinner("训练线性 softmax 分类器..."):
        w, _, losses, acc, source = train_linear_classifier(samples, 13, steps, lr, reg)
    with c1:
...
def page_optimizer_loss():
    ...
    with b:
        ...
        for i in range(5):
            scores.append(st.slider(f"score[{i}]", -5.0, 5.0, defaults[i], 0.1))
